...
void bt1036c_config(const struct device *uart, const uint8_t txrx_config)
{
    ...
    */
    ...
    if(txrx_config != BT103036C_CONFIG_RX)
    {
        ...
        while(bt1036c_status.scan[0] == 0);

        bt1036c_at_send("A2DPCONN=DC0D300022E4"); // Hardwired MACs address
        k_sleep(K_MSEC(2000));
...
static int decode_bt1036c_data(char *s)
{
    ...
    {
        strncpy(bt1036c_status.scan, data, sizeof(bt1036c_status.scan)-1);
        return 0;
    }
