...
#define VOLUME_LEV 1
#define MY_SAMPLE_FREQ 44100
#define MY_NUM_BLOCKS 4 // Each block is a buffer (2 buffers; 1 to read and 1 to write simultaneosly + 2 backup buffers)
#if (STEREO == 1)
#define MY_CHANNELS_NUMBER 2
#else
#define MY_CHANNELS_NUMBER 1
#endif

/* Computations */
#define AMPLITUDE (VOLUME_REF * VOLUME_LEV)
#define SAMPLE_NO (MY_SAMPLE_FREQ / TONE_FREQ)
#define CHUNK_DURATION (float)((float)SAMPLE_NO * (float)MY_NUM_BLOCKS / (float)MY_SAMPLE_FREQ)
...
#define MY_BLOCK_SIZE (MY_CHANNELS_NUMBER * sizeof(sin_data))
 ...
 */
K_MEM_SLAB_DEFINE(my_slab, MY_BLOCK_SIZE, MY_NUM_BLOCKS, 4);

static void generate_sine_wave(void);
static void fill_buf(int16_t *my_tx_block);

void *my_tx_block[MY_NUM_BLOCKS] = {0}; // Pointer to the blocks
 ...
 * @return int
 */
int i2s_sample_config(const struct device *dev_i2s)
{
    ...
    i2s_cfg.channels = MY_CHANNELS_NUMBER;
    i2s_cfg.format = I2S_FMT_DATA_FORMAT_I2S;
    i2s_cfg.frame_clk_freq = MY_SAMPLE_FREQ;
    i2s_cfg.block_size = MY_BLOCK_SIZE;
    i2s_cfg.timeout = 2000;
    i2s_cfg.options = I2S_OPT_FRAME_CLK_MASTER | I2S_OPT_BIT_CLK_MASTER;
    i2s_cfg.mem_slab = &my_slab;
...
int i2s_sample(const struct device *dev_i2s)
{
    ...
    for (tx_idx = 0; tx_idx < MY_NUM_BLOCKS; tx_idx++)
    {
        /* One block allocated for each cycle */
        if (k_mem_slab_alloc(&my_slab, &my_tx_block[tx_idx], K_FOREVER) < 0)
        {
            printf("Failed to allocate TX block\n");
            return -1;
        }
        /* Fill each block with data */
        fill_buf((int16_t *)my_tx_block[tx_idx]);
    }

    /* Write initial block (needed by I2S to know at least one block is ready)*/
    if (i2s_write(dev_i2s, my_tx_block[tx_idx++], MY_BLOCK_SIZE) < 0)
    {
...
static void generate_sine_wave(void)
{
    ...
    float sample_rate = (float)MY_SAMPLE_FREQ;
...
static void fill_buf(int16_t *my_tx_block)
{
    ...
    {
#if (STEREO == 1)
        my_tx_block[2 * i] = sin_data[i]; // Left channel
        my_tx_block[2 * i + 1] = 0;       // Right channel
#if (0)
        /* Fake right channel */
        int r_idx = (i + SAMPLE_NO / 4) % SAMPLE_NO;
        my_tx_block[2 * i + 1] = sin_data[r_idx]; // Right channel (90° shifted)
#endif
#else
        my_tx_block[i] = sin_data[i]; // Left channel
#endif
    }
}